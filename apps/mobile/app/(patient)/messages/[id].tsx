...
export default function MessageThreadScreen() {
  const { id: partnerId, name: partnerName, speciality: partnerSpeciality } = useLocalSearchParams<{ id: string; name: string; speciality?: string }>()
  ...
  useEffect(() => {
    ...
    supabase.from('messages')
      ...
      .then(() => { void queryClient.invalidateQueries({ queryKey: ['conversations', profile.id] }) })
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#f8f9ff' }} edges={['top']}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: 16, paddingVertical: 12, backgroundColor: 'rgba(255,255,255,0.90)', borderBottomWidth: 1, borderBottomColor: 'rgba(226,232,240,0.5)' }}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={{ fontSize: 11, color: '#6f787e', fontFamily: 'Manrope' }}>
            {partnerSpeciality ?? 'Professionnel de santé'}
          </Text>
        </View>
      ...
      {/* Medical disclaimer */}
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 16, paddingVertical: 10, backgroundColor: '#fff8e1', borderBottomWidth: 1, borderBottomColor: 'rgba(226,232,240,0.4)' }}>
        <MaterialIcons name="info-outline" size={14} color="#705d00" />
        <Text style={{ flex: 1, fontSize: 11, color: '#705d00', fontFamily: 'Manrope', lineHeight: 16 }}>
          Réservé aux échanges médicaux. En cas d'urgence, appelez le <Text style={{ fontWeight: '800' }}>15</Text>.
        </Text>
      </View>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <View style={{ backgroundColor: 'rgba(255,255,255,0.95)', borderTopWidth: 1, borderTopColor: 'rgba(226,232,240,0.5)', paddingHorizontal: 12, paddingTop: 10, paddingBottom: Platform.OS === 'ios' ? 12 : 10 }}>
          ...
          >
            ...
            <Text style={{ fontSize: 12, fontWeight: '600', color: '#82d8ff', fontFamily: 'Manrope' }}>
      ...
      <Modal visible={docPickerVisible} transparent animationType="slide" onRequestClose={() => setDocPickerVisible(false)}>
        ...
        <View style={{ backgroundColor: '#fff', borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingHorizontal: 20, paddingTop: 12, paddingBottom: 32 }}>
          ...
          <Text style={{ fontSize: 16, fontWeight: '800', color: '#0b1c30', fontFamily: 'Manrope', marginBottom: 4 }}>
            Quel type de document ?
          </Text>
          <Text style={{ fontSize: 12, color: '#6f787e', fontFamily: 'Manrope', marginBottom: 16 }}>
