...
export default function OrganizationMessageThreadScreen() {
  ...
  const { data: messages = [], isLoading } = useQuery<Message[]>({
    queryKey: ['org-thread', profile?.id, id],
  ...
  const send = useMutation({
    ...
    onSuccess: () => {
      setText('')
      qc.invalidateQueries({ queryKey: ['org-thread', profile?.id, id] })
      qc.invalidateQueries({ queryKey: ['org-conversations', profile?.id] })
    },
  })
              ...
              return (
                <View style={{ alignItems: isMe ? 'flex-end' : 'flex-start' }}>
                  <View style={{
                    ...
                    <Text style={{ fontSize: 14, fontFamily: 'Manrope', color: '#0b1c30' }}>{item.body}</Text>
                  </View>
