...
const ROLE_LABELS: Record<string, string> = {
  admin: 'Équipe M-Santé', organization_admin: "Admin d'org", organization_member: 'Collaborateur', secretary: 'Secrétaire',
}
...
function NewMessageModal({ myId, onClose }: { myId: string; onClose: () => void }) {
  ...
  const { data: directory = [], isLoading } = useQuery<DirectoryUser[]>({
    queryKey: ['org-internal-directory', myId],
    queryFn: async () => {
      const { data: me } = await supabase.from('users').select('organization_id').eq('id', myId).single()
      const { data, error } = await supabase
        .from('users')
        .select('id, full_name, role')
        .or(`role.eq.admin,organization_id.eq.${me?.organization_id}`)
        .order('full_name')
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                onPress={() => {
                  onClose()
                  router.push({ pathname: '/(organization)/messages/[id]', params: { id: item.id, name: item.full_name } })
                }}
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#f8f9ff' }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: 20, paddingVertical: 14, backgroundColor: 'rgba(255,255,255,0.70)', borderBottomWidth: 1, borderBottomColor: 'rgba(226,232,240,0.5)' }}>
        ...
        <Text style={{ flex: 1, fontSize: 18, fontWeight: '800', color: '#0b1c30', fontFamily: 'Manrope' }}>Messages</Text>
      ...
      {isLoading ? (
        ...
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', gap: 12, paddingHorizontal: 40 }}>
          ...
          <Text style={{ fontSize: 13, color: '#6f787e', fontFamily: 'Manrope', textAlign: 'center', lineHeight: 20 }}>
            Échangez avec l&apos;équipe M-Santé et vos collaborateurs.
          </Text>
        </View>
      ) : (
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              onPress={() => router.push({ pathname: '/(organization)/messages/[id]', params: { id: item.partnerId, name: item.partnerName } })}
              activeOpacity={0.7}
